...
def get_trained_model():
    predictor = DemandPredictor(data_path='data/')  # Upewnij się co do ścieżki
    with st.spinner('Wczytywanie danych i trening modelu... (To zajmie chwilę)'):
        predictor.load_data()
        predictor.train()
    return predictor
...
with col1:
    ...
    selected_product_category = st.selectbox("Wybierz kategorię:", product_list)

    # Pobranie danych z naszego obiektu
    details = predictor.get_product_details(selected_product_category)
...
with col2:
    ...
    pred_demand = predictor.predict_demand(new_price, details['competitor_price'])
...
if api_key:
    if st.button("Poproś o analizę strategiczną"):
        ...
        Oceń zwięźle (max 3 punkty):
        1. Opłacalność ruchu.
        2. Ryzyko (np. wojna cenowa).
        3. Rekomendacja (Tak/Nie/Modyfikuj).
        """
...
demands = [predictor.predict_demand(p, details['competitor_price']) for p in prices]
